<template>
  <div class="simulation-panel">
    ...
    <div class="events-summary">
      <span class="events-label">TOTAL SIGNALS:</span>
      ...
      <span class="events-platform">Plant Floor <span class="events-count">{{ runStatus.twitter_actions_count || 0 }}</span></span>
      <span class="events-slash">/</span>
      <span class="events-platform">Supplier Desk <span class="events-count">{{ runStatus.reddit_actions_count || 0 }}</span></span>
      <span class="events-slash">/</span>
      <span class="events-platform">Risk Market <span class="events-count">{{ runStatus.polymarket_actions_count || 0 }}</span></span>
    </div>
    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed, selected: filteredPlatform === 'twitter', dimmed: filteredPlatform && filteredPlatform !== 'twitter' }" @click="filterByPlatform('twitter')">
          ...
          <div class="platform-actions-list"><span class="action-tag">ASSESS</span><span class="action-tag">ACK</span><span class="action-tag">ESCALATE</span><span class="action-tag">NOTE</span><span class="action-tag">ALIGN</span></div>
        </div>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed, selected: filteredPlatform === 'reddit', dimmed: filteredPlatform && filteredPlatform !== 'reddit' }" @click="filterByPlatform('reddit')">
          ...
          <div class="platform-actions-list"><span class="action-tag">UPDATE</span><span class="action-tag">REVIEW</span><span class="action-tag">ACK</span><span class="action-tag">FLAG</span><span class="action-tag">SEARCH</span><span class="action-tag">ALIGN</span></div>
        </div>
        ...
        <div class="platform-status polymarket" :class="{ active: runStatus.polymarket_running, completed: runStatus.polymarket_completed, selected: filteredPlatform === 'polymarket', dimmed: filteredPlatform && filteredPlatform !== 'polymarket' }" @click="filterByPlatform('polymarket')">
          ...
          <div class="platform-actions-list"><span class="action-tag">SCAN</span><span class="action-tag">HEDGE+</span><span class="action-tag">HEDGE-</span><span class="action-tag">PRICE</span><span class="action-tag">NOTE</span></div>
        </div>
      </div>
    </div>
    ...
    <div class="main-content-area" ref="scrollContainer" @scroll="onTimelineScroll">
      ...
      <div v-if="filteredPlatform" class="agent-filter-bar">
        <div class="filter-info">
          <span class="filter-name" :class="filteredPlatform">{{ filteredPlatform === 'twitter' ? 'Plant Floor' : filteredPlatform === 'reddit' ? 'Supplier Desk' : 'Risk Market' }}</span>
          <span class="filter-count">{{ chronologicalActions.length }} signals</span>
        </div>
              ...
              <div class="card-header">
                ...
                <div class="header-meta">
                  <div class="platform-indicator" :class="action.platform">
                    <span class="lane-pill">{{ action.platform === 'twitter' ? 'Plant Floor' : action.platform === 'reddit' ? 'Supplier Desk' : 'Risk Market' }}</span>
                  </div>
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.twitter_current_round > prevTwitterRound.value) {
        addLog(`[Plant Floor] R${data.twitter_current_round}/${data.total_rounds} | T:${data.twitter_simulated_hours || 0}h | A:${data.twitter_actions_count}`)
        prevTwitterRound.value = data.twitter_current_round
      }
      
      if (data.reddit_current_round > prevRedditRound.value) {
        addLog(`[Supplier Desk] R${data.reddit_current_round}/${data.total_rounds} | T:${data.reddit_simulated_hours || 0}h | A:${data.reddit_actions_count}`)
...
const getActionTypeLabel = (type) => {
  const labels = {
    ...
    'REPOST': 'ESCALATE',
    'LIKE_POST': 'ACK',
    'CREATE_COMMENT': 'REVIEW',
    'LIKE_COMMENT': 'ACK',
    'DISLIKE_POST': 'FLAG',
    'DISLIKE_COMMENT': 'FLAG',
    'MUTE': 'MUTE',
    'DO_NOTHING': 'IDLE',
    'FOLLOW': 'ALIGN',
    'SEARCH_POSTS': 'SEARCH',
