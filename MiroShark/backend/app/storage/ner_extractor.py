...
_SYSTEM_PROMPT = """You are a Named Entity Recognition and Relation Extraction system for an operational supply-chain simulation.
Given a text and an ontology (entity types + relation types), extract all entities and relations relevant to factory operations, suppliers, logistics, customers, regulators, materials, and mitigation actions.
...
Example input: "Northstar Battery Components has 18 days of lithium precursor inventory. Ganfeng Lithium supplies the precursor from China. A new export restriction doubles lead times."
Example output:
{{
  "entities": [
    {{"name": "Northstar Battery Components", "type": "Factory", "attributes": {{"location": "Nevada"}}}},
    {{"name": "lithium precursor", "type": "Material", "attributes": {{"inventory_days": "18"}}}},
    {{"name": "Ganfeng Lithium", "type": "Supplier", "attributes": {{"country": "China", "material_role": "lithium precursor"}}}},
    {{"name": "Chinese export restriction", "type": "Regulator", "attributes": {{"policy_area": "trade controls"}}}}
  ],
  "relations": [
    {{"source": "Northstar Battery Components", "target": "lithium precursor", "type": "DEPENDS_ON", "fact": "Northstar Battery Components depends on lithium precursor inventory for production."}},
    {{"source": "Ganfeng Lithium", "target": "lithium precursor", "type": "SUPPLIES", "fact": "Ganfeng Lithium supplies lithium precursor from China."}},
    {{"source": "Chinese export restriction", "target": "Ganfeng Lithium", "type": "REGULATES", "fact": "A Chinese export restriction affects Ganfeng Lithium shipments and doubles lead times."}}
  ]
}}

Example input: "A port disruption delays separator film shipments by 14 days. Seika Separator Systems reroutes cargo through Long Beach. Northstar's procurement lead requests expedite freight."
Example output:
{{
  "entities": [
    {{"name": "separator film", "type": "Material", "attributes": {{}}}},
    {{"name": "Seika Separator Systems", "type": "Supplier", "attributes": {{"material_role": "separator film"}}}},
    {{"name": "Long Beach", "type": "LogisticsNode", "attributes": {{"node_type": "port"}}}},
    {{"name": "Northstar procurement lead", "type": "ProcurementLead", "attributes": {{"role": "Procurement Lead"}}}}
  ],
  "relations": [
    {{"source": "Seika Separator Systems", "target": "separator film", "type": "SUPPLIES", "fact": "Seika Separator Systems supplies separator film."}},
    {{"source": "Seika Separator Systems", "target": "Long Beach", "type": "SHIPS_TO", "fact": "Seika Separator Systems reroutes cargo through Long Beach."}},
    {{"source": "Northstar procurement lead", "target": "separator film", "type": "MITIGATES", "fact": "Northstar's procurement lead requests expedite freight to reduce separator film delays."}}
  ]
}}
